以下是 `Task`/`Milestone`/`ProgressTracker` 类的完整实现代码，仅依赖 Python 内置库 (`json`/`uuid`/`os`)，满足进度跟踪和 JSON 持久化需求：
...
### **代码说明**
1. **数据模型**：
   - `Task` 支持嵌套子任务（`subtasks`）
   - `Milestone` 包含进度和时间统计方法
   - 所有类实现 `to_dict()`/`from_dict()` 用于 JSON 序列化

2. **持久化**：
   - `ProgressTracker` 自动加载/保存 JSON 文件
   - 支持递归查找任务（跨里程碑和子任务）
...
3. **使用方式**：
   ...
   # 添加里程碑
   milestone = Milestone("GUI开发")
   milestone.add_task(Task("PyQt界面设计", time_planned=10))
   tracker.milestones.append(milestone)
   
   # 更新任务进度
   task = tracker.find_task("目标任务的ID")
   if task:
...
下一步可基于这些类开发 `tkinter` GUI 界面。需要输出界面代码吗？